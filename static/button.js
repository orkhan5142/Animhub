...
<button class="button-shutter-in">Shutter</button>`
    ...
    "button-arrow-circle": {
        ...
        html: `<button class="button-arrow-circle"><span class="icon-arrow">→</span></button>`,
        css: `/* Arrow Circle Button Effect */
.button-arrow-circle {
  position: relative;
  width: 60px;
  height: 60px;
...
.button-arrow-circle:hover .icon-arrow {
  color: white;
}
.button-arrow-circle::before {
  ...
  border-radius: 50%;
  transform: translate(-50%, -50%);
  transition: width 0.4s, height 0.4s cubic-bezier(0.23, 1, 0.32, 1);
  z-index: 0;
}
.button-arrow-circle:hover::before {
  width: 100%;
  height: 100%;
}`,
        js: `// No JavaScript required for this animation`,
...
<button class="button-arrow-circle"><span class="icon-arrow">→</span></button>`
    },
};